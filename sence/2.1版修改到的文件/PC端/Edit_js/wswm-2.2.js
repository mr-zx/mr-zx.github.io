...
//==============================华丽分割线30====撤销恢复 begin=================================20160123 增加模块
/* ----------撤销、恢复机制详解：---------------
 * 建立两个栈，一个是撤销栈，一个恢复栈，都存储的是元素的html()；撤销栈存储用户所有导致变化后的html，
 * 恢复栈存储用户撤销后的操作，相当于撤销栈出栈的元素添加到恢复栈；
 * 当用户的操作导致变化，恢复栈要入栈元素，同时恢复栈不为空时，要清空恢复栈；
 * 撤销操作会撤销到撤销栈的（index-1）的索引对应的数据
 * 恢复操作会恢复到恢复栈的（ index ）的索引对应的数据
 * 撤销栈默认数据是初始化后的状态数据
 * 恢复栈默认数据为空
 */
//初始化撤销栈
function initStack() {
    $("#undo").data("index", 0);
    $("#redo").data("index", 0);
    //undoIn(Phone.html()); //缓存拷贝对象
    $("#undo").data("phone" + 0, Phone.html()); //缓存拷贝对象  
    stackUi();
}
function stackUi() {
    var $undo = $("#undo");
    ...
    if (!$redo.data("index")) {
        ...
        if (!$redo.hasClass('active'))
            $redo.addClass('active');
    }
    console.log($undo.data("index") + ": " + $redo.data("index"))
}
function undoIn(data) {
    var $undo = $("#undo");
    var index = $undo.data("index");
    index = index + 1; //记录次数
    $undo.data("index", index);
    $undo.data("phone" + index, data); //缓存拷贝对象    
}
function undoOut() {
    var $undo = $("#undo");
    var index = $undo.data("index");
    //if (!index) return "";
    var data = $undo.data("phone" + index);
    $undo.data("phone" + index, "").data("index", (index - 1));
    return data;
}
function redoIn(data) {
    var $redo = $("#redo");
    var index = $redo.data("index");
    index = index + 1; //记录次数
    $redo.data("index", index);
    $redo.data("phone" + index, data); //缓存拷贝对象
}
function redoOut() {
    var $redo = $("#redo");
    var index = $redo.data("index");
    if (!index) return "";
    var data = $redo.data("phone" + index);
    $redo.data("phone" + index, "").data("index", index - 1);
    return data;
}
function redoClear() {
    var $redo = $("#redo");
    ...
    for (var i = 0; i < index; i++) {
        $redo.data("phone" + index, "")
    }
    $redo.data("index", 0);
}
function undoCurrentData() {
    return $("#undo").data("phone" + $("#undo").data("index"));
}
//执行操作后，有变化的话，就放入undo的数据栈里
function stackInChange() {
    if (undoCurrentData() != Phone.html()) {
        undoIn(Phone.html());
    }
    if ($("#redo").data("index") != '0') {
        redoClear();
    }
    stackUi();
}
//撤销操作
function undoAction() {
    if ($("#undo").data("index") == '0') return;
    var data = undoOut();
    redoIn(data);
    Phone.html(undoCurrentData());
    initPhone();
    stackUi();
}
//恢复操作
function redoAction() {
    if ($("#redo").data("index") == '0') return;
    var data = redoOut();
    undoIn(data);
    Phone.html(data);
    initPhone();
